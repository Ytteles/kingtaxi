...
<?php 
    ...
    // запаска фильтрация только по дате
    // $order = "`order_id`,`order_data`, `locationIn`, `locationOut`, `user_number` ORDER BY `order_id` ASC";
    //  $sql = "SELECT `order_data`, `order_id`, `locationIn`, `locationOut`, `user_number` FROM `order` WHERE {$dateCond} GROUP BY {$order}";


    //SELECT `order_data`, `order_id`, `locationIn`, `locationOut`, `user_number`, class_taxi`, `comment` FROM `order` WHERE `order_data` BETWEEN '1001-02-21' AND '2023-10-10' GROUP BY `order_id`,`locationIn`, `locationOut`, `user_number`, class_taxi`, `comment` ORDER BY `order_id` ASC;
    $order = "`order_id`,`order_data`, `locationIn`, `locationOut`, `user_number`, `comment` ORDER BY `order_id`";
    //$sql = "SELECT `order_data`, `order_id`, `locationIn`, `locationOut`, `user_number`, `comment`, `class_taxi` FROM `order` WHERE `order_data` BETWEEN '1001-02-21' AND '2023-10-10' GROUP BY `order_id`,`locationIn`, `locationOut`, `user_number` ORDER BY `order_id` ASC;";
    ...
    // $sqlxml = "SELECT `order_data`, `order_id`, `locationIn`, `locationOut`, `user_number`, `comment`, `class_taxi`
    // FROM `order` WHERE {$dateCond} AND {$classTaxi} GROUP BY {$order} {$IDvozr} FOR XML AUTO;";


    
    
    
    //$sql = "SELECT `order_data`, `order_id`, `locationIn`, `locationOut`, `user_number`, `class_taxi`,`comment` FROM `order` WHERE {$classTaxi} AND {$dateCond} GROUP BY {$order} ASC;";
    // $stmtxml = pdo()->prepare($sqlxml);
    //$stmtxml->execute();
    // $xml = $stmtxml;
...
<body class="feedback__body">
    ...
    <header class="head__fed">
        <nav class="header__nav">
            ...
            <li class="menu__item"><a href="applications.php" class="menu__link">Заявки</a></li>
    ...
    <div>
        <H2 class="h2__style">История заказов</H2>
